...
| 🔴 | Verify that an expiry of 0 still reaches the glasses | Blocked on the glasses. The notification lives about a millisecond on the phone, and the whole feature rests on the listener that feeds the glasses being told at post rather than at cancel. If it turns out not to reach them, 0 is a broken setting rather than a fast one, and [`DESIGN.md`](DESIGN.md) "What the user can change" needs a different mechanism. Done when it has been seen on the glasses with expiry 0 |
