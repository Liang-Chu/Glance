...
| --- | --- | --- |
| 🟡 | Decide what the app actually asks the LLM for: whether the prompt is fixed by the app or editable by the user, and what a keyword becomes inside it | Blocked on the owner deciding. With retrieval gone this prompt is the entire product — it is the only thing standing between a keyword and what the wearer reads. Done when [`DESIGN.md`](DESIGN.md) "Where a tip comes from" states it with a criterion |
| 🟡 | Decide how far back the memory of recent tips reaches, and whether a repeat is rejected outright or merely discouraged | Blocked on the owner deciding. A model handed its own recent output can still repeat itself, so this is a quality problem the design has not closed. Done when [`DESIGN.md`](DESIGN.md) states the depth and the rejection rule with a criterion |
