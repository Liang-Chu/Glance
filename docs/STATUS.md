...
Stage: **it works end to end on a phone** — a notification carrying the backend's own words has been
seen. Whether it reaches the glasses is still unrecorded · last full sweep 2026-09-10
...
| Backend client | `app/src/main/java/dev/liamchu/glance/Backend.kt` | **live** | `./gradlew.bat testDebugUnitTest` drives it against a real loopback HTTP server across every outcome; and it has now called a real backend from a phone, 2026-09-10 |
...
| Notification | `app/src/main/java/dev/liamchu/glance/Notifier.kt` | **live** | the owner saw the title and text the backend returned, 2026-09-10 |
...
| Backend contract | [`CONTRACT.md`](CONTRACT.md) | **live** | every clause names the test that exercises it or says it has none, and a real backend has now answered it, 2026-09-10 |
