...
| Scheduled run and manual check | `app/src/main/java/dev/liamchu/blip/BlipWorker.kt`, `Scheduler.kt` | **built, never run** | compiles; WorkManager pulled `RECEIVE_BOOT_COMPLETED` into the merged manifest, which is what would carry it across a reboot, 2026-09-10 |
