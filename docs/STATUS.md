...
Stage: **built, never run** — the app compiles and packages; no part of it has been exercised on a
device or against a backend · last full sweep 2026-09-10
...
| Settings screen | `app/src/main/java/dev/liamchu/blip/MainActivity.kt` | **built, never run** | compiles; never opened on a device, 2026-09-10 |
| Backend client | `app/src/main/java/dev/liamchu/blip/Backend.kt` | **built, never run** | compiles; never called against any backend, 2026-09-10 |
...
| Backend contract | [`CONTRACT.md`](CONTRACT.md) | **specified, unexercised** | no implementation on the other side has ever answered it, 2026-09-10 |
...
**"Built, never run" is not "works".** `C9` counts the running thing, and for this project the
running thing is a notification readable on the glasses. Nothing in the table above has met that bar.
